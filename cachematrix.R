...
makeCacheMatrix <- function(x = matrix()) {
      ...
      set <- function(y) {
        ...
        ## parent environment
        t <<- NULL ## re-initialize m in the parent environment to null
      }
      get <- function() x ## return the matrix x
      setinverse <- function(inverse) t <<- inverse ## set the cache m equal
      ## to the inverse of the matrix x
      getinverse <- function() t ## return the cached inverse of x
